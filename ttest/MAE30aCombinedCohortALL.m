% Code for MAE30A Winter 25 T-test All Survey Questions Combined Cohort (A00 and B00)
...
val18_boq = convert(boq_data{:,18});
val29_boq = convert(boq_data{:,29});
val43_boq = convert(boq_data{:,43});
...
boq_data_fil = boq_data(attn_checked_boq, :);

% For EOQ
% Convert the attention-check responses to numeric
val18_eoq = convert(eoq_data{:,18});
val29_eoq = convert(eoq_data{:,29});
val43_eoq = convert(eoq_data{:,43});
...
eoq_data_fil = eoq_data(attn_checked_eoq, :);
